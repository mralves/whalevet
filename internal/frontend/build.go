...
func Build(ctx context.Context, c client.Client) (_ *client.Result, err error) {
	...
	// whalevet: apply Dockerfile injection rules passed via
	// FrontendOpt "proxy.rules" (base64-encoded JSON of []config.Injection),
	// with CA cert contents from "proxy.certs" inlined (no context files).
	src.Data = applyProxyRules(src.Data, opts)
...
func warnOpts(r []parser.Range, detail [][]byte, url string) client.WarnOpts {
	...
	for _, r := range r {
		opts.Range = append(opts.Range, &pb.Range{
			Start: &pb.Position{
				Line:      int32(r.Start.Line),      //nolint:gosec // Dockerfile line numbers are small
				Character: int32(r.Start.Character), //nolint:gosec // Dockerfile column numbers are small
			},
			End: &pb.Position{
				Line:      int32(r.End.Line),      //nolint:gosec // Dockerfile line numbers are small
				Character: int32(r.End.Character), //nolint:gosec // Dockerfile column numbers are small
			},
		})
	}
	return opts
